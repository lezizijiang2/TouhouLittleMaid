...
public class AnimationRegister {
    ...
    public static void registerAnimationState() {
        ...
        register("jump", Priority.NORMAL, (maid, event) -> !maid.asEntity().isOnGround() && !maid.asEntity().isInWater());

        register("run", Priority.LOW, (maid, event) -> maid.asEntity().isOnGround() && maid.asEntity().isSprinting());
        register("walk", Priority.LOW, (maid, event) -> maid.asEntity().isOnGround() && event.getLimbSwingAmount() > MIN_SPEED);
