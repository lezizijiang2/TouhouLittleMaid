...
public abstract class AbstractModelGui<T extends LivingEntity, E extends IModelInfo> extends Screen {
    ...
    public AbstractModelGui(T entity, List<CustomModelPack<E>> listPack) {
        super(TComponent.literal("Custom Model GUI"));
        this.entity = entity;
    ...
    public void init() {
        ...
        // 添加开启、关闭模型包图标缓存的按钮
        Component enableCache = TComponent.translatable("gui.touhou_little_maid.skin.enable_cache");
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        blit(poseStack, middleX - 256 / 2, middleY - 80, 0, 0, 256, 180);
        RenderSystem.setShader(GameRenderer::getPositionTexShader);
        RenderSystem.setShaderTexture(0, SIDE);
        blit(poseStack, middleX - 256 / 2 + 250, middleY - 80, 0, 0, 24, 180);

        // 绘制侧边的滚动条
        drawScrollSide(poseStack, middleX, middleY);

        // 调用父类方法绘制按钮列表、标签列表
        drawButton(poseStack, mouseX, mouseY, partialTicks);

        // 绘制标签栏图标
        drawTabIcon(poseStack, middleX, middleY);
        ...
        // 绘制实体，绘制完毕后绘制文本提示
        drawEntity(poseStack, middleX, middleY);

        // 绘制其他文本提示
        drawTooltips(poseStack, mouseX, mouseY, middleX, middleY);
        ...
        for (int index = 0; index < size; index++) {
            ...
            if (icon != null) {
                ...
                if (pack.getIconAnimation() == CustomModelPack.AnimationState.FALSE) {
                    ...
                    blit(poseStack, middleX - 92 + 28 * index, middleY - 98,
                            0, 0, 16, 16, 16, 16);
                } else {
                    RenderSystem.setShader(GameRenderer::getPositionTexShader);
                    RenderSystem.setShaderTexture(0, icon);
    ...
    private void drawEntity(PoseStack poseStack, int middleX, int middleY) {
        ...
        // 绘制包信息
        drawPackInfoText(poseStack, pack, middleX, middleY);
    ...
    private void drawPackInfoText(PoseStack poseStack, CustomModelPack<E> pack, int middleX, int middleY) {
        ...
        for (FormattedText properties : packSplitName) {
            offsetY += 10;
            drawCenteredString(poseStack, font, properties.getString(), sideMiddleX, middleY + offsetY, 0xffffff);
        }
        ...
        for (Component str : ParseI18n.parse(pack.getDescription())) {
            ...
            for (FormattedText properties : split) {
                offsetY += 10;
                drawCenteredString(poseStack, font, properties.getString(), sideMiddleX, middleY + offsetY, ChatFormatting.DARK_GRAY.getColor());
            }
        }
        ...
        // 绘制最后的翻页数
        drawCenteredString(poseStack, font, String.format("%s/%s", getPageIndex() + 1, guiNumber.getPageSize()), middleX, middleY - 120, 0xffffff);
    }
    ...
    private void drawTooltips(PoseStack poseStack, int mouseX, int mouseY, int middleX, int middleY) {
        ...
        for (E modelItem : pack.getModelList().subList(fromIndex, toIndex)) {
            ...
            if (isxInRange && isyInRange) {
                ...
                if (getMinecraft().options.advancedItemTooltips) {
                    tooltips.add(TComponent.literal(modelItem.getModelId().toString()).withStyle(ChatFormatting.DARK_GRAY));
                }
        ...
        for (int index = 0; index < size; index++) {
            ...
            if (isxInRange && isyInRange) {
                ...
                renderTooltip(poseStack, ParseI18n.parse(hoverPack.getPackName()), mouseX, mouseY);
            }
        }
