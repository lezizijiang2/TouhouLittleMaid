...
public class STTChatKey {
    ...
    @SubscribeEvent
    public static void onSttChatPress(InputEvent.KeyInputEvent event) {
        if (STT_CHAT_KEY.matches(event.getKey(), event.getScanCode())) {
            ...
            if (event.getAction() == GLFW.GLFW_RELEASE) {
                ...
                if (maids.isEmpty()) {
                    player.sendMessage(TComponent.translatable("ai.touhou_little_maid.stt.content.no_maid_found"), Util.NIL_UUID);
                } else {
    ...
    private static void sttStart() {
        ...
        if (AIConfig.CHAT_ENABLED.get()) {
            ...
            if (StringUtils.isBlank(url)) {
                player.sendMessage(TComponent.translatable("ai.touhou_little_maid.chat.stt.empty"), Util.NIL_UUID);
            } else {
                STTClient sttClient = Service.getSttClient(url);
                sttClient.start(message -> {
                }, throwable -> {
                    String cause = throwable.getLocalizedMessage();
                    player.sendMessage(TComponent.translatable("ai.touhou_little_maid.stt.connect.fail")
                            .append(cause).withStyle(ChatFormatting.RED), Util.NIL_UUID);
                });
            }
        }
    }
    ...
    private static void sttStop(EntityMaid maid) {
        ...
        sttClient.stop(message -> {
            ...
            if (StringUtils.isNotBlank(chatText)) {
                ...
                player.sendMessage(TComponent.literal(format).withStyle(ChatFormatting.GRAY), Util.NIL_UUID);
            } else {
                player.sendMessage(TComponent.translatable("ai.touhou_little_maid.stt.content.empty").withStyle(ChatFormatting.GRAY), Util.NIL_UUID);
            }
        }, throwable -> {
            String cause = throwable.getLocalizedMessage();
            player.sendMessage(TComponent.translatable("ai.touhou_little_maid.stt.connect.fail")
