...
public class EntityFairy extends Monster implements RangedAttackMob, FlyingAnimal, IHasPowerPoint {
    ...
    public static boolean checkFairySpawnRules(EntityType<EntityFairy> entityType, ServerLevelAccessor levelAccessor, MobSpawnType spawnType, BlockPos pos, Random randomSource) {
        if (Monster.checkMonsterSpawnRules(entityType, levelAccessor, spawnType, pos, randomSource) && levelAccessor instanceof ServerLevel level) {
            ...
            long findCount = level.getPoiManager().getInSquare(type -> type.equals(InitPoi.SCARECROW.get()), pos, scarecrowRange, PoiManager.Occupancy.ANY).count();
            return findCount <= 0;
        }
        return false;
    }
