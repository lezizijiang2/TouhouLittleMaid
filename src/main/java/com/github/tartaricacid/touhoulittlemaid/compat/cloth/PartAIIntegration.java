...
public class PartAIIntegration {
    ...
    public static ConfigBuilder getConfigBuilder(EntityMaid maid) {
        ...
        root.setSavingRunnable(() -> PacketDistributor.sendToServer(new SaveMaidAIDataPackage(maid.getId(), maid.getAiChatManager())));
        return root;
    }
    ...
    public static void init(ConfigBuilder root, ConfigEntryBuilder entryBuilder, EntityMaid maid) {
        ...
        String defaultLanguage = StringUtils.isBlank(manager.getTtsLanguage()) ? DEFAULT_LANGUAGE : manager.getTtsLanguage();
        ai.addEntry(entryBuilder.startStringDropdownMenu(Component.translatable("config.touhou_little_maid.part_ai.tts_language"),
                ...
                .setDefaultValue(DEFAULT_LANGUAGE).setTooltip(Component.translatable("config.touhou_little_maid.part_ai.tts_language.tooltip"))
