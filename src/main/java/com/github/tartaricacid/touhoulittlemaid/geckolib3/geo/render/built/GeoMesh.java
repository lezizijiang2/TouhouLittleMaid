...
public class GeoMesh {
    ...
    static public class GeoMeshBuilder {
        ...
        private static Quaternion fromZYX(float pX, float pY, float pZ) {
        ...
        public void addCube(Cube cubeIn, ModelProperties properties, Double boneInflate, Boolean mirror) {
            ...
            Vector3f size = new Vector3f(VectorUtils.fromArray(cubeIn.getSize()));
            size.mul(0.0625f);
            Vector3f origin = new Vector3f(VectorUtils.fromArray(cubeIn.getOrigin()));
            origin.mul(0.0625f);
            ...
            Quaternion quat = fromZYX(rotation.z(), rotation.y(), rotation.x());

            poseMatrix.translate(new Vector3f(pivot.x(), pivot.y(), pivot.z()));
            poseMatrix.multiply(quat);
            poseMatrix.translate(new Vector3f(-pivot.x(), -pivot.y(), -pivot.z()));

            mulPosition(P1, poseMatrix);

            POSITION[index].set(new float[]{P1.x(), P1.y(), P1.z()});
        ...
        private Vector3f mulPosition(Vector3f in, Matrix4f mat) {
